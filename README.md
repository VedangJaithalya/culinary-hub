...
supported way to run this — there's no deployed instance to point at.
...
no runtime deps beyond React itself for the UI layer.
...
Two standalone HTML files in `dashboards/` — no build step, no server, open
either directly in a browser:
...
- **`culinaryfeed-dashboard.html`** — an internal-style leadership dashboard:
  ...
  no charting library, no network calls.
...
personal build, run locally — not deployed, not handling production traffic.
